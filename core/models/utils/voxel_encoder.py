...
class VoxelGenerator(object):
    ...
    Args:
        ...
        max_num_points_per_voxel (int): Maximum number of points in a single voxel
        max_num_voxels (int, optional): Maximum number of voxels.
            Defaults to 20000.
    """
    ...
    def generate(self, points):
        ...
        return points_to_voxel(points, self._voxel_size,
                               self._point_cloud_range, self._max_num_points_per_voxel,
                               True, self._max_num_voxels)
    ...
    @property
    def max_num_points_per_voxel_per_voxel(self):
        """int: Maximum number of points per voxel."""
        return self._max_num_points_per_voxel
    ...
    def __repr__(self):
        ...
        repr_str += indent + f'max_num_points_per_voxel={self._max_num_points_per_voxel},\n'
        repr_str += indent + f'max_num_voxels={self._max_num_voxels},\n'
...
def points_to_voxel(points,
    ...
    Args:
        ...
        reverse_index (bool): Whether return reversed coordinates. \
            ...
            xyz format.
        max_num_voxels (int): Maximum number of voxels this function creates. \
            For second, 20000 is a good choice. Points should be shuffled for \
            randomness before this function because max_num_voxels drops points.
    ...
    voxels = np.zeros(
        shape=(max_num_voxels, max_points, points.shape[-1]), dtype=points.dtype)
    coors = np.zeros(shape=(max_num_voxels, 3), dtype=np.int32)
    if reverse_index:
        voxel_num = _points_to_voxel_reverse_kernel(
            points, voxel_size, coors_range, num_points_per_voxel,
            coor_to_voxelidx, voxels, coors, max_points, max_num_voxels)

    else:
        voxel_num = _points_to_voxel_kernel(points, voxel_size, coors_range,
                                            num_points_per_voxel,
                                            coor_to_voxelidx, voxels, coors,
                                            max_points, max_num_voxels)
...
def _points_to_voxel_reverse_kernel(points,
    ...
    Args:
        ...
        max_points (int): Indicate maximum points contained in a voxel.
        max_num_voxels (int): Maximum number of voxels this function create. \
            for second, 20000 is a good choice. Points should be shuffled for \
            randomness before this function because max_num_voxels drops points.
