...
def import_file(path, name: str = None, add_to_sys=True,
    ...
    if not disable_warning:
        logger.warning((
            "Failed to perform regular import for file {0}. "
            ...
            "or add __init__.py to ensure this file can be regularly imported. ".format(
                path)
        ))
    ...
    if add_to_sys:  # this will enable find objects defined in a file.
        ...
        if module_name in sys.modules and module_name not in CUSTOM_LOADED_MODULES:
            raise ValueError("{0} exists in system.".format(module_name))
