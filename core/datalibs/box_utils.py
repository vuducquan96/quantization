...
from core.ops.roiaware_pool3d import roiaware_pool3d_utils
...
def boxes_to_corners_3d(boxes3d):
    ...
    Returns:
    """
    if isinstance(boxes3d, list) == True:
...
def boxes3d_kitti_camera_to_lidar(boxes3d_camera, calib):
    ...
    l, h, w = boxes3d_camera_copy[:, 3:4], boxes3d_camera_copy[:,
                                                               4:5], boxes3d_camera_copy[:, 5:6]
...
def boxes3d_kitti_fakelidar_to_lidar(boxes3d_lidar):
    ...
    w, l, h = boxes3d_lidar_copy[:, 3:4], boxes3d_lidar_copy[:,
...
def boxes3d_kitti_lidar_to_fakelidar(boxes3d_lidar):
    ...
    """
    boxes3d_lidar_copy = copy.deepcopy(boxes3d_lidar)
    dx, dy, dz = boxes3d_lidar_copy[:,
                                    3:4], boxes3d_lidar_copy[:, 4:5], boxes3d_lidar_copy[:, 5:6]
...
def boxes_iou_normal(boxes_a, boxes_b):
    ...
    iou = a_intersect_b / \
        torch.clamp_min(area_a[:, None] + area_b[None,
                        :] - a_intersect_b, min=1e-6)
    return iou
...
def boxes3d_lidar_to_aligned_bev_boxes(boxes3d):
    ...
    """
    rot_angle = common_utils.limit_period(
        boxes3d[:, 6], offset=0.5, period=np.pi).abs()
